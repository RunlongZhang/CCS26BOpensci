...
public:
    ...
    //placeholder for estimating storage
    void populateLeaf(int n, int dim, bool firstIt, CompactDataset ds, ITreeNode* node = nullptr)
    {
        if (!node && !firstIt)
        {
            return;
        ...
        {
            node->Fl.reserve(n / 2);
            node->Fr.reserve(n - (n / 2));

            // Use a pointer to the raw data instead of a queue copy
            const int8_t* rawData = ds.data.data();

            for (int i = 0; i < n; i++)
            {
                const int8_t* currentBlock = rawData + (i * dim);
                std::string func;
                func.reserve(dim);
                for (int j = 0; j < dim; j++)
                {
                    func += static_cast<char>('0' + currentBlock[j]);
                }
                if (i < (n / 2))
                {
                    node->Fl.push_back(func);
                }
                else
                {
                    node->Fr.push_back(func);
                }
            }
        }
    }
    ...
    size_t estimateStorage(int tnodes, int tleaf, int trleaf, int dim, int n, int ads)
    {
        ...
        size_t funcBytes = (size_t)trleaf * strPerLeaf * bytesPerString;
        ...
        return metad + pstore + mstore + funcBytes;
    }
};